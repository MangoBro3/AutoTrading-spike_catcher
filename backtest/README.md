...
- `core/runner.py`: Hybrid 시뮬레이터 기반 실행기
- `core/hybrid_simulator.py`: 상태머신+배분+가드 연동 시뮬레이션 엔진
...
> NOTE: 기본 실행은 `core/hybrid_simulator.py`를 사용한다.
> 실데이터 엔진 연동 시에는 `EngineAdapter`를 주입해 동일 산출물 계약을 유지하면 된다.
